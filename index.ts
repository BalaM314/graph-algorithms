...
const graph1 = new Graph<DData>((node, edge) => {
	const base = node();
	const t1 = node();
	const t2 = node();
	const t3 = node();
...
const graph2 = new Graph<AData>((node, edge) => {
	const begin = node();
	const a = node();
	const b = node();
	const c = node();
...
const graph2heuristic = (node:GNode<unknown>) => ({
	begin: 12,
	a: 8,
	b: 6,
	c: 7,
...
/** Data stored in each node to run Dijkstra's algorithm. */
type DData = [final:number | null, working:number[]];

function dijkstra(graph:Graph<DData>, source:string, target:string):GNode<DData>[] {
	let sourceNode:GNode<DData> = graph.nodesNamed.get(source) ?? crash("Invalid source node");
	const targetNode:GNode<DData> = graph.nodesNamed.get(target) ?? crash("Invalid target node");
	...
	while(currentNode != sourceNode){
		for(const [node, edge] of currentNode.connections()){
			if(node.value[0] === null || currentNode.value[0] === null) impossible();
...
/** Data stored in each node to run the A* algorithm. */
type AData = {
	movementCost: number | null;
	movementCostFinalized: boolean;
	totalScore: number | null; //f = g + h
};
type ADataFinal = AData & { movementCost: number; };

function aStar(graph:Graph<AData>, heuristic:(node:GNode<AData>) => number, source:string, target:string):GNode<AData>[] {
	let sourceNode:GNode<AData> = graph.nodesNamed.get(source) ?? crash("Invalid source node");
	const targetNode:GNode<AData> = graph.nodesNamed.get(target) ?? crash("Invalid target node");
	...
	let reversedPath:GNode<AData>[] = [targetNode];
	let currentNode2 = targetNode;
	nextNode:
	while(currentNode2 != sourceNode){
		for(const [node, edge] of currentNode2.connections()){
			if(!node.value.movementCostFinalized || node.value.movementCost == null) continue; //this node was not explored, so it cannot be part of the path
			if(currentNode2.value.movementCost == null) impossible();
